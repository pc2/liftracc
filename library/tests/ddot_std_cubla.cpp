...
*/
    ...
    for (i=1; i<=max_exp; i++) {
      int p = (int)pow(2, i);
        ...
        double ret = cublasDdot(p, dev_a, 1, dev_b, 1);
...
#ifdef _LIFTRACC_PROFILING_        
    print_profiling_data("cubla_ddot_result", &(liftracc_profiling_data[0]), prof_id_names, ID_COUNT);
    print_profiling_data("cubla_ddot_result", &(data[0]), 0, max_exp+1);
#endif /* _LIFTRACC_PROFILING_ */
