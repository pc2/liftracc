...
*/
    ...
    // function init
    double (*atlas_daxpy)(const int n, const double alpha, const double * x, const int incx, double * y, const int incy);

    *(void **) (&atlas_daxpy) = dlsym(atlas_handle, "ATL_daxpy");
...
#endif /* _LIFTRACC_PROFILING_ */
        (*atlas_daxpy)(p, alpha, dA, 1, dB, 1);
#ifdef _LIFTRACC_PROFILING_        
        liftracc_function_timing_stop(&(data[i]));
#endif /* _LIFTRACC_PROFILING_ */
        MSG("atlas_%d_%d.run", p, j);
      }
   }

#ifdef _LIFTRACC_PROFILING_
    print_profiling_data("atlas_daxpy_result", &(liftracc_profiling_data[0]), prof_id_names, ID_COUNT);
    print_profiling_data("atlas_daxpy_result", &(data[0]), 0, max_exp+1);
#endif /* _LIFTRACC_PROFILING_ */
