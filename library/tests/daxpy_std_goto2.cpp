...
*/
    ...
    if (goto2_handle) INFO("lib init ok.");

    // function init
    double (*goto2_daxpy)(const int n, const double alpha, const double * x, const int incx, double * y, const int incy);

    *(void **) (&goto2_daxpy) = dlsym(goto2_handle, "cblas_daxpy");
...
#endif /* _LIFTRACC_PROFILING_ */
        (*goto2_daxpy)(p, alpha, dA, 1, dB, 1);
#ifdef _LIFTRACC_PROFILING_        
        liftracc_function_timing_stop(&(data[i]));
#endif /* _LIFTRACC_PROFILING_ */
        MSG("goto2_%d_%d.run", p, j);
    }
   }

#ifdef _LIFTRACC_PROFILING_        
    print_profiling_data("goto2_daxpy_result", &(liftracc_profiling_data[0]), prof_id_names, ID_COUNT);
    print_profiling_data("goto2_daxpy_result", &(data[0]), 0, max_exp+1);
#endif /* _LIFTRACC_PROFILING_ */
