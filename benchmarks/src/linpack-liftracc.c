...
int main(void)
{
    ...
    for (arsize=2; arsize<=MAX_DIM; arsize*=2) {
        arsize/=2;
        arsize*=2;
    ...
    *info = 0;
	...
	if (nm1 >=  0)
		for (k = 0; k < nm1; k++) {
			kp1 = k + 1;
			/* find l = pivot index */
			l = liftracc_idamax(n-k,&a[lda*k+k],1) + k;
			ipvt[k] = l;
			...
			if (a[lda*k+l] != ZERO) {
				...
				/* compute multipliers */
				t = -ONE/a[lda*k+k];
				liftracc_dscal(n-(k+1),t,&a[lda*k+k+1],1);
				...
				for (j = kp1; j < n; j++) {
					...
					liftracc_daxpy(n-(k+1),t,&a[lda*k+k+1],1,&a[lda*j+k+1],1);
				}
			} else
				(*info) = k;
		}
	...
	if (job == 0) {
		...
		if (nm1 >= 1)
			for (k = 0; k < nm1; k++) {
				...
				liftracc_daxpy(n-(k+1),t,&a[lda*k+k+1],1,&b[k+1],1);
			}
		/* now solve  u*x = y */
		for (kb = 0; kb < n; kb++) {
			k = n - (kb + 1);
			b[k] = b[k]/a[lda*k+k];
			t = -b[k];
			liftracc_daxpy(k,t,&a[lda*k+0],1,&b[0],1);
		}
	} else {
		/* job = nonzero, solve  trans(a) * x = b  */
		/* first solve  trans(u)*y = b             */
		for (k = 0; k < n; k++) {
			t = liftracc_ddot(k,&a[lda*k+0],1,&b[0],1);
			b[k] = (b[k] - t)/a[lda*k+k];
		}
		...
		if (nm1 >= 1)
			for (kb = 1; kb < nm1; kb++) {
				k = n - (kb+1);
				b[k] = b[k] + liftracc_ddot(n-(k+1),&a[lda*k+k+1],1,&b[k+1],1);
				l = ipvt[k];
				if (l != k) {
					t = b[l];
					b[l] = b[k];
